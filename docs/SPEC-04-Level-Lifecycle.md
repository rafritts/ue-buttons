...
`_state` against the actual level — already landed in SPEC-03 (`scene op=reconcile`). This
...
Driving Level 1 in a fresh `Untitled_2`, `view(map)` reported **2 paths / 4 scatters** when
...
- **`level op=clear`** — delete all ueb-spawned actors + foliage in the current level
  (scoped by the `ueb` tag / `ueb_scatter:` component tags, exactly as `scene`/`scatter
  remove` already scope), leaving the engine scaffolding. The "wipe my arrangement, keep the
  map" button.
...
into `view(map)`/`scene` default reads (the `_prune_dead_intents` pattern) so a phantom
...
- A `level` op that changes the loaded map returns the old and new level names, and the
  ...
  cleaned.
- `clear` reports the count removed by kind (actors / populations), like `scatter remove`.
