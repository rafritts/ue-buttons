"""`landscape` — terrain as a heightfield (SPEC-01 E3).
...
`remove` action (or editor delete), not `history undo_to`. Lying about undo is worse than a
documented gap.
"""
import json
import os
...
def _rebuild(actor, meta):
    ...
    feats = meta["features"]
    disp = [unreal.Vector(0.0, 0.0, base + terrain.height_at(p.x, p.y, feats, extent))
            for p in positions]
...
def _meta_of(label):
    return _state.landscapes.get(label)
